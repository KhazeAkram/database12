...
--
-- Host: 127.0.0.1
-- Generation Time: May 04, 2023 at 08:44 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `teacher_course`  AS SELECT `teacher`.`Teacher_ID` AS `Teacher_ID`, `courses`.`C_ID` AS `C_ID` FROM (`teacher` left join `courses` on(`teacher`.`Teacher_ID` = `courses`.`C_ID`))  ;
